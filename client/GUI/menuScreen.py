...
def draw ():
    ...
    frame1.pack(side="top",pady=5)
    entryBox = Entry(frame1, bg = "#E9F4EE", fg = "#000000", font = fontWord, justify = LEFT, bd = 15, width = 80) #validate = "key", validatecommand=(mainClient.register(validate_input), "%S")
    entryBox.pack(side="left", padx=10,pady=5)
    buttonConnect = Button(frame1, text = "Connect", font = fontWord, width = 25, bg = COLOUR_BUTTON, fg = COLOUR_FONT, padx = 40, pady = 10, command=lambda: get_ip(entryBox))
    buttonConnect.pack(side="left", padx=10,pady=5)
    change_frame(frame1)
    frame3 = ttk.Frame(mainClient)
    frame3.pack(side="left",padx=5,pady=5)
    change_frame(frame3)
    buttonProcess = Button(frame3, text = "Process Running", font = fontWord, width = 30,height =4, bg = COLOUR_BUTTON, fg = COLOUR_FONT,command = lambda: pcs_window(),padx = 20, pady = 20)
    buttonProcess.pack(side="top", pady=5)
    buttonApp = Button(frame3, text = "App Running", font = fontWord, width = 30, height =4, bg = COLOUR_BUTTON, fg = COLOUR_FONT, command = lambda: app_window(),padx = 20, pady = 20)
    buttonApp.pack(side="top",pady=5)
    frame10 = ttk.Frame(mainClient)
    frame10.pack(side="left",padx=5)
    change_frame(frame10)
    frame5 = ttk.Frame(frame10)
    frame5.pack(side="top",pady=5)
    change_frame(frame5)
    buttonTurnOff = Button(frame5, text = "Turn-Off\n\nComputer", font = fontWord, width = 30, height =4, bg = COLOUR_BUTTON, fg = COLOUR_FONT, command = lambda: click_button("shutdown"), padx = 20, pady = 20)
    buttonTurnOff.pack(side="top",pady=5)
    buttonCap = Button(frame5, text = "Print\n\nScreen", font = fontWord, width = 30, height =4, bg = COLOUR_BUTTON, fg = COLOUR_FONT,command = lambda: scr_window(), padx = 20, pady = 20)
    buttonCap.pack(side="top",pady=5)
    ...
    frame8.pack(side="top",pady=5)
    change_frame(frame8)
    buttonKeyStroke = Button(frame8, text = "Keystroke", font = fontWord, width = 30, height =4, bg = COLOUR_BUTTON, fg = COLOUR_FONT,command = lambda: kst_window(), padx = 20, pady = 20)
    buttonKeyStroke.pack(side="top",pady=5)
    buttonRegistry = Button(frame8, text = "Disconnect Server", font = fontWord, width = 30, height =4, bg = COLOUR_BUTTON, fg = COLOUR_FONT, command = lambda: disconnect_work("disconnect"), padx = 20, pady = 20)
    buttonRegistry.pack(side="top",pady=5)
    mainClient.mainloop()
